...
async function fetchData(projectId: string): Promise<ApiResp> {
  ...
  const url = `${proto}://${host}/api/data/${projectId}/donor`;
  ...
  if ("error" in data) {
    return (
      <main style={{ maxWidth: 860, margin: "48px auto", padding: 16 }}>
        <h1>Project {params.projectId} — Donor</h1>
        {data.error === "auth" ? (
          <>
          ...
          <p>Forbidden: you don’t hold the Donor access token for this project.</p>
        ) : (
...
<h1>Donor view — Project {params.projectId}</h1>
<p className="muted">You’ll see Project data for your chosen beneficiary project project if your wallet holds the investor access token.</p>
